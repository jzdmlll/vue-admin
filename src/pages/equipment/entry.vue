...
export default {
  ...
  data() {
    ...
    return {
      ...
      rule: {
        ...
        firstParty: [{ required: true, message: "不能为空", trigger: "blur" }],
        secondParty: [{ required: true, message: "不能为空", trigger: "blur" }],
  ...
  methods: {
    ...
    getFile(record) {
      getAction("/chapter/chapterAudit/findFileUrlByOtherId", {
        id: record,
      }).then((resp) => {
